...
class Mymodel_generate:
    ...
    def generate_fixed_task(self, exist_task, generate_file, train_loader):
        ...
        for idx, data in enumerate(train_loader):
            ...
            annotator_id = data[3].numpy().astype(np.int64)
            label_np = data[5].numpy()



            if task_id in list(map(int, exist_task)):  # current_task在集合中 直接保存
                f_save.write(str(task_id[0][0]) + '\t' + str(annotator_id[0][0]) + '\t' + str(int(label_np[0][0])) + '\n')
            else:
                ...
                label_new = int(np.random.choice(np.array(range(4)), p=p.ravel()))
                f_save.write(str(task_id[0][0]) + '\t' + str(annotator_id[0][0]) + '\t' + str(label_new) + '\n')
        f_save.close()
